...
export default function MovieSelectionPage() {
    ...
    // 스토어에서 기존 영화 목록 가져오기
    const { movies: storedMovies } = useOnboardingStore();
    ...
    useEffect(() => {
        const loadMovies = async () => {
            ...
            // ✅ 이미 저장된 영화가 있으면 API 호출하지 않고 재사용
            if (storedMovies && storedMovies.length > 0) {
                console.log("✅ 저장된 영화 목록 사용 (API 호출 생략):", storedMovies);
                setMovies(storedMovies);
                setIsLoading(false);
                return;
            }

            // ⚠️ 저장된 영화가 없을 때만 API 호출
            try {
                ...
                console.log("✅ 영화 로딩 성공 (새로 불러옴):", moviesData);
            } catch (err: any) {
        ...
        loadMovies();
    }, [setGlobalMovies, storedMovies]); // clearMovieSelection 의존성 제거
